...
본 프로젝트의 최종 목적은 **Mac M4 Pro 64GB 환경에서 최신 Qwen3 모델 시리즈를 활용한 AutoGen 멀티 에이전트 시스템 구축**입니다.

- **Architect (설계자)**: `qwen3-coder-next:q4_K_M` - 전체 시스템 설계 및 로직 가이드 제공.
- **Coder (개발자)**: `qwen3-coder:32b` - 실제 코드 구현 및 TDD 기반 개발 수행.
- **Reviewer (검수자)**: `qwen3-coder:14b` - 코드 리뷰 및 품질 검증.

이를 통해 기획부터 구현, 검증까지 자동화된 개발 파이프라인을 로컬 환경에서 완벽하게 구현하는 것을 목표로 합니다.
