...
> **⚠️ 경고: Cursor의 직접 연동 설정(Base URL Override)은 지연 및 연결 오류 등 '문제가 많으므로' 권장되지 않습니다.**
> 회사망에서 로컬 LLM을 사용하려는 경우, Cursor 앱 자체 설정보다는 **Continue** 확장 프로그램을 설치하고 **[12. Continue 로컬 LLM 최적화 설정](./12_Continue_로컬_LLM_최적화_설정.md)** 가이드를 따르는 것이 훨씬 안정적입니다.
...
3. **회사 노트북**의 **Continue** 설정에서 `http://100.x.x.x:8000` 대신 **`https://your-node.tail-net.ts.net`** 을 `apiBase`로 설정합니다. (Cursor 자체 설정은 권장하지 않습니다.)
...
1. **개인 PC**에서 Tailscale Funnel을 켜서 HTTPS 주소를 생성한 뒤, **Continue** 확장 프로그램에서 해당 주소로 접속해 봅니다. (Cursor 자체 설정은 지연 및 오류가 잦으므로 피하세요.)
