# 10. Cursor + ngrok 로컬 Ollama 연동 가이드 (비권장: 문제가 많음)

> **⚠️ 경고: 현재 이 연동 방식은 '문제가 많다'고 판단되어 권장되지 않습니다.**
> 커서(Cursor) 앱 자체의 `Override OpenAI Base URL` 설정은 응답 지연, 모델 인식 오류, 그리고 보안 터널링 설정의 번거로움으로 인해 실제 개발 환경에서 불안정합니다.
> 대신 **[12. Continue 로컬 LLM 최적화 설정](./12_Continue_로컬_LLM_최적화_설정.md)** 가이드를 따라 **Continue** 확장 프로그램을 사용하는 방식을 강력히 권장합니다.
