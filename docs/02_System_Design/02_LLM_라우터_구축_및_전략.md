...
라우터는 에이전트와 LLM 엔진(Ollama) 사이의 중재자 역할을 수행하며 다음 네 가지 핵심 기능을 담당합니다.

1.  **작업 분류 (Task Classification)**: 요청된 작업이 설계, 코딩, 검수 중 어디에 해당하는지 판단합니다.
2.  **모델 라우팅**: 작업 성격에 맞는 최적의 Qwen3 모델을 호출합니다.
3.  **메모리 오케스트레이션**: 모델 로드/언로드를 제어하여 64GB 메모리 임계치를 넘지 않도록 관리합니다.
4.  **헬스 모니터링**: Ollama 상태 및 추론 지연 시간(Latency)을 실시간으로 체크합니다.
