...
3. [최종 판단 및 워크플로우 결정]:
   - [난이도: 상]: 시스템 전반의 아키텍처 재설계, 5개 이상의 파일 간 의존성 수정, 2026년형 보안 프로토콜 적용. 
     -> **전략: 52B Planner 호출 + Reviewer(14B) 필수 검토 루프 활성화.**
   - [난이도: 중]: 복잡한 로직 구현, 다중 모듈 수정, 참조 문서의 50% 이상을 반영해야 하는 경우.
     -> **전략: Dev1(32B) 할당 + Reviewer(14B) 1회 자아성찰 루프 활성화.**
   - [난이도: 하]: 단순 최신 문법 교체, 단일 모듈 리팩토링, 제공된 스니펫의 단순 적용. 
     -> **전략: Dev1(32B) 즉시 할당 (검토 루프 생략, 속도 우선).**
...
- **성능 최적화**: 난이도 '하'인 경우에는 리뷰어 소환 없이 즉시 답변을 내보내어 응답 속도를 최대화하라.
"""
```
