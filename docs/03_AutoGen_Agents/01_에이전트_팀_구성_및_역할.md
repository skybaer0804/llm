...
### 🛰️ Router (Gateway)
- **모델**: `qwen2.5:7b`
- **역할**: 시스템의 '지능형 게이트웨이', 비서 및 **1차 보안 검문소(Input Guardrail)**.
- **임무**: 사용자 요청 분석, 에이전트 라우팅, 컨텍스트 요약, 결과 포맷 검수. **외부 데이터(문서, 이슈 등) 내의 악성 프롬프트 주입 여부 탐지**.
- **특징**: 초경량 모델을 사용하여 항시 상주하며 시스템 반응 속도를 최적화합니다. **보안 위협 감지 시 즉시 Human_Lead에게 보고**.
...
- **모델**: `qwen3-coder:14b`
- **역할**: 코드 품질 최적화, Best Practice 적용 검수 및 **최종 보안 가이드레일(Output Guardrail)**.
- **임무**: 최종 리팩토링 및 보안 검수(SQL Injection, 자원 유출, **간접 프롬프트 주입 반영 여부**), Pythonic 코드 유도. 
- **특징**: Tester와 모델을 공유하며, 호출 시점에 페르소나를 전환합니다. **자가 성찰 루프를 통해 생성된 결과물의 안전성을 최종 확약합니다.**
