...
- ARCHITECT: 새로운 기능 설계, 프로젝트 구조 변경, 대규모 리팩토링이 필요할 때. (메모리 스왑 발생)
- CODER: 기존 설계 위에서 실제 코드를 작성하거나 수정할 때.
- TESTER: 작성된 코드를 검증하거나 버그 리포트를 분석할 때.

응답은 반드시 다음 JSON 형식으로만 해:
{"next_agent": "AGENT_NAME", "reason": "이유", "requires_swap": true/false}
```
...
1. [FIX_CODE]: 단순 논리 오류나 문법 에러일 때 (CODER에게 전달, swap: false)
2. [RE_ARCHITECT]: 현재 설계로는 테스트 통과가 불가능한 구조적 결함일 때 (ARCHITECT에게 전달, swap: true)
3. [ENV_ERROR]: 라이브러리 미설치나 경로 문제 등 환경 오류일 때 (USER에게 조치 요청)

응답은 반드시 JSON: {"action": "ACTION_NAME", "analysis": "에러 원인 요약", "requires_swap": boolean}
```
